...
| 상황 | 처리 |
|---|---|
| 승자를 못 가린 경기 | 결과 모름(`unknown`)으로 등록 — 홈페이지와 같다(래더에서 빠지고 통계는 전적·승률만 안 셈) |
| 회원과 안 이어지는 참가자 | 비회원 슬롯으로 등록 |
| 이미 등록된 경기(시작 시각 일치) | 기존 경기에 지표·맵·시간 머지. 서버가 지금 것보다 긴 저장본이면 파일도 갈아 끼우고 다시 굽는다 |
| 컴퓨터(AI)가 낀 경기 | 등록(사이트 검토 화면 기본값과 같음) |
| **회원이 2명 미만인 경기** | 건너뜀 — 양 팀 합쳐 서로 다른 회원 2명(꼭 한 팀에 한 명씩일 필요는 없다). 혼자 비회원·컴퓨터하고만 한 판을 거른다. 홈페이지 검토 화면의 "자동 제외"와 같은 기준 |
| 유즈맵(게임 갈래 10) | 건너뜀(서버가 거절하는 갈래) |
| 팀을 두 편으로 못 나눔 | 건너뜀 |
| 관전자로 추정해 뺀 사람이 있음 | 건너뜀 |
| 2분 미만 경기 | 건너뜀 |
| 한쪽 팀이 비었거나 종족을 못 읽음 | 건너뜀(난투(FFA)는 승자 자리(team1)가 비어도 등록) |
...
체크아웃이 있어야 한다)로 다시 맞춘다 — 손으로 고치지 않는다. 홈페이지가 재생기 패키지(scplay)에서
빌려 쓰는 타입 두 개(`Race`·`ReplayMapGrid`)만 `web/src/types/scplay.d.ts`에 따로 베껴 둔다(등록기 소유).
`src/pipeline.ts`는 홈페이지 `replayDraft.ts`의 사본이라(그 파일은 브라우저 API 클라이언트를
끌어들여 그대로 못 쓴다) 동기화할 때 그 둘도 견주어 본다.
...
등록기는 Electron 메인(Node)에서 서버를 부르므로 CORS는 걸리지 않는다(브라우저 밖이다).
