...
// 알려진 컴퓨터/비회원 이름 자동 분류 → 중복이면 머지, 아니면 등록. 홈페이지의
// src/utils/replayDraft.ts와 같은 순서·같은 필드지만, 검토 모달이 사람에게 묻던 것은
// 전부 config.ts의 고정 규칙으로 정한다(승자 없음 → 승패모름, 미매칭 → 비회원).
//
...
import { newUnregisteredSlotId } from "../web/src/constants/unregisteredSlot";
import type { GameOutcome, GameResultSlot, Member, NewGameResult } from "../web/src/types";
...
export async function processReplay(path: string, api: UploaderApi, members: Member[]): Promise<Outcome> {
  ...
  if (parsed.teamSplitUncertain) return skip("팀을 두 편으로 못 나눔(UMS 등)");
  ...
  if (parsed.team1.length === 0 || parsed.team2.length === 0) return skip("한쪽 팀이 비어 있음");
  if ([...parsed.team1, ...parsed.team2].some((p) => !p.isComputer && !p.race)) return skip("종족을 못 읽은 참가자 있음");
  if (RULES.skipIfComputer && [...parsed.team1, ...parsed.team2].some((p) => p.isComputer)) return skip("컴퓨터 낀 경기");
  ...
  // ── 중복이면 머지(지표·맵·시간만 갱신, 승패는 확실할 때만) ──
  ...
  // ── 등록 — 승자를 못 가렸으면 승패모름(not_held) ──
  const result: GameOutcome = parsed.winnerSide ?? "not_held";
